...
// grLayer is locked for the root window, however child windows may use a different viewport (e.g. extruding menu)
...
// Call when we are expected to land on the Main grLayer (0) after FocusWindow()
...
static void ImGui::NavUpdate()
{
    ...
    // Store our return window (for returning from grLayer 1 to grLayer 0) and clear it as soon as we step back in our own grLayer 0
    if (g.NavWindow)
...
static void ImGui::NavUpdateInitResult()
{
    ...
    IMGUI_DEBUG_LOG_NAV("[nav] NavInitRequest: result NavID 0x%08X in grLayer %d Window \"%s\"\n", g.NavInitResultId, g.NavLayer, g.NavWindow->Name);
...
static void ImGui::NavUpdateMoveResult()
{
    ...
    IMGUI_DEBUG_LOG_NAV("[nav] NavMoveRequest: result NavID 0x%08X in grLayer %d Window \"%s\"\n", result->ID, g.NavLayer, g.NavWindow->Name);
